...
def TracerFieldCross2DA(kMax, BinsRad, MaskedQueryPosRad, MaskedTracerPosRad, SmoothedFieldDict,  FieldConstPercThreshold, Verbose=False):
    ...
    '''
    Returns the probabilities :math:`P_{>k}`, :math:`P_{>dt}` and :math:`P_{>k,>dt}` for :math:`1 \leq k \leq` `kMax`, that quantify the extent of the spatial cross-correlation between the given discrete tracer positions (`MaskedTracerPosRad`) and the given continuous overdensity field. 
    	
        1. :math:`P_{>k}(\theta)`: 
    		the kNN-CDF of the discrete tracers, evaluated at angular distance scale :math:`\theta`
    		
    	2. :math:`P_{{\rm dt}}(\theta)`: 
    		the probability of the overdensity field smoothed with a top-hat filter of angular size :math:`\theta` exceeding the given constant percentile density threshold
    		
    	3. :math:`P_{>k, {\rm dt}}(\theta)`:
    		the joint probability of finding at least 'k' tracers within a spherical cap of radius :math:`\theta` AND the overdensity field smoothed at angular scale :math:`\theta` exceeding the given density threshold
    		
    The excess cross-correlation (Banerjee & Abel 2023) [1]_ can be computed trivially from the quatities (see the 'HelperFunctions' module for a method to do this):
    	
    	.. math:: \psi_{k, {\rm dt}} = P_{>k, dt}/(P_{>k} \times P_{{\rm dt}})
    ...
    MaskedQueryPosRad : array_like of shape (n_query, 2)
        array of sky locations for the query points. The sky locations must be on a grid. For each query point in the array, the first (second) coordinate should be the declination (right ascension) in radians. Please ensure -pi/2 <= declination <= pi/2 and 0 <= right ascension <= 2*pi.
    MaskedTracerPosRad : array_like of shape (n_tracer, 2)
        array of sky locations for the discrete tracers. For each query point in the array, the first (second) coordinate should be the declination (right ascension) in radians. Please ensure -pi/2 <= declination <= pi/2 and 0 <= right ascension <= 2*pi.
    ...
    Returns
    -------
    p_gtr_k_list: list of array_like, each of shape len(`BinsRad`[k-1]) for 1 <= k <= `kMax`
        auto kNN-CDFs of the discrete tracers evaluated at the desired distance bins.
        
    p_gtr_dt_list: list of array_like, each of shape len(`BinsRad`[k-1]) for 1 <= k <= `kMax`
        continuum version of auto kNN-CDFs for the continuous field evaluated at the desired 
        distance bins.
    
    p_gtr_k_dt_list: list of array_like, each of shape len(`BinsRad`[k-1]) for 1 <= k <= `kMax`
    ...
    Notes
    -----
    Measures the angular cross-correlation between a set of discrete tracers and a continuous overdensity field using the k-nearest neighbour (kNN) formalism as defined in Banerjee & Abel (2023) [1]_ and Gupta & Banerjee (2024) [2]_.
    ...
    Data with associated observational footprints are supported, in which case, only tracer positions within the footprint should be provided and the field should be masked appropriately.Importantly, in this case, query points need to be within the footprint and appropriately padded from the edges of the footprint (see Gupta & Banerjee (2024) [2]_ for a detailed discussion). If the footprints of the tracer set and the field are different, a combined mask representing the intersection of the two footprints should be used (see the 'HelperFunctions' module for help with masking and creating the modified query positions).

    References
    ----------
    .. [1] Arka Banerjee, Tom Abel, Tracer-field cross-correlations with k-nearest neighbour    
       distributions, Monthly Notices of the Royal Astronomical Society, Volume 519, Issue 4, March 2023, Pages 4856–4868, `link <https://doi.org/10.1093/mnras/stac3813>`_
        
       [2] Kaustubh Rajesh Gupta, Arka Banerjee, Spatial clustering of gravitational wave sources with k-nearest neighbour distributions, Monthly Notices of the Royal Astronomical Society,Volume 531, Issue 4, July 2024, Pages 4619–4639, `link <https://doi.org/10.1093/mnras/stae1424>`_
    '''
